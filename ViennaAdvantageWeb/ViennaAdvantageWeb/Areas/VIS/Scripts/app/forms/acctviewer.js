; (function (VIS, $) {
    ...
    function AcctViewerData(windowNu, ad_Client_ID, ad_Table_ID) {
        ...
        var displayQty = false;
        var displaySourceAmt = false;
        var displayDocumentInfo = false;
        ...
        this.ASchemas = getClientAcctSchema(this.AD_Client_ID);
        ...
        function getClientAcctSchema(AD_Client_ID) {
            var obj = [];
    ...
    AcctViewerData.prototype.Query = function (AD_Client_ID) {
        //  Set Where Clause
        var whereClause = "";
        //  Add Organization
        ...
        this.getDataModel(AD_Client_ID, whereClause, orderClause, this.group1, this.group2, this.group3, this.group4, this.sortBy1, this.sortBy2, this.sortBy3, this.sortBy4);
        var val = this.dataByData;
        return val;
    }

    AcctViewerData.prototype.getDataModel = function (AD_Client_ID, whereClause, orderClause, gr1, gr2, gr3, gr4, sort1, sort2, sort3, sort4) {
        var obj = this;
        $.ajax({
    ...
    //form declaretion
    function AcctViewer(AD_Client_ID, AD_Table_ID, Record_ID, windowNum) {
            ...
            for (var i = 0; i < dataObj.Data.length ; i++) {
                ...
                for (var j = 0; j < dataObj.Columns.length; j++) {
                    ...
                    if (row[j] != null && typeof (row[j]) == "object") {
                        ...
                        if (row[j] != null && dataObj.Columns[j] == "Account Date") {
        ...
        function initializeComponent() {
            ...
            topDiv = $("<div id='" + "topDiv_" + windowNo + "' style='float: left; width: 100%; height: 45px; margin-bottom: 2px;margin-left: 0px;'>" +
                      ...
                      "<label id='" + "lblresult_" + windowNo + "' class='VIS_Pref_Label_Font' style='vertical-align: middle; cursor: pointer;'>" + VIS.Msg.getMsg("ViewerResult") + "</label></div></div>");
        ...
        function dynInit(AD_Table_ID, Record_ID) {
            ...
            actionDocument();
            actionTable();
            lblstatusLine.getControl().text(VIS.Msg.getMsg("EnterSelctionAndDisplayToQueryFind"));
            lblstatusLine.getControl().css("color", "#19A0ED");//css("font-size", "28px").
            ////  Initial Query
            if (haveDoc) {
