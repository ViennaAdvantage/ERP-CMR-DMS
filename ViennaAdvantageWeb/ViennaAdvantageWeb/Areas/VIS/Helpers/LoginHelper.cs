    ...
    public class LoginHelper
    {
        ...
        public static bool Login(LoginModel model, out List<KeyNamePair> roles)
        {
            ...
            StringBuilder sql = new StringBuilder("SELECT u.AD_User_ID, r.AD_Role_ID,r.Name,")
               ...
               .Append(" WHERE (COALESCE(u.LDAPUser,u.Name)=@username OR COALESCE(u.LDAPUser,u.Value)=@username)")
            ...
            if (model.Login1Model.Password != null)
                sql.Append(" AND (u.Password='" + model.Login1Model.Password + "' OR u.Password='" + SecureEngine.Encrypt(model.Login1Model.Password) + "')");	//  #2/3
