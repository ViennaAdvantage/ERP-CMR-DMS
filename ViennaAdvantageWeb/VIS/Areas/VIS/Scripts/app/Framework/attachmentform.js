    ...
    function attachmentForm(windowNo, AD_Attachment_ID, AD_Table_ID, Record_ID, trxName, newRecord_ID, restrictUpload, isDMSAttachment) {
        ...
        function LoadFileAttachment() {
            ...
            $.ajax({
                ...
                error: function () {
                    alert(VIS.Msg.getMsg('ERRORGettingAttachment'));
                    bsyDiv[0].style.visibility = "hidden";
                },
                success: function (data) {
                    ...
                    var btnUpload = $("<a class='vis-attach-ico vis-browse-ico' style='margin-top:-12px'>");
                    ...
                    var btnuploadFiles = $("<a href='javascript:void(0)'  class='vis-attach-ico vis-uploadall-ico' style='height:30px;width:30px;margin-right: 7px;' >");
                    dLABtns.append(btnuploadFiles);
                    btnuploadFiles.on('click', function () {
                        UploadFiles();
                    });

                    var btnCancelAll = $("<a href='javascript:void(0)'  class='vis-attach-ico vis-cancelall-ico' style='margin-right: 10px;height:30px;width:30px;'>");
                    ...
                    var btnDeleteAll = $('<a class="vis-btn" style="margin-left: 5px;cursor: pointer;">').append(VIS.Msg.getMsg('DeleteAll'));
                    ...
                    btnDeleteAll.on('click', function () {
                        ...
                        if (VIS.ADialog.ask("DeleteRecord?")) {
                            ...
                            $.ajax({
                                ...
                                error: function () {
                                    alert(VIS.Msg.getMsg('ERRORDeletingFile'));
                                    bsyDiv[0].style.visibility = "hidden";
                                },
                                success: function (data) {
                                    if (data.result == -1) {
                    ...
                    var btnCancel = $('<a class="vis-btn"  style="margin-left: 5px;cursor: pointer;">').append(VIS.Msg.getMsg('Cancel'));
                ...
                for (var itm in lstLatestFiles) {
                    if (file.name == lstLatestFiles[itm].name) {
                        window.setTimeout(function () {
                            alert(VIS.Msg.getMsg('fileAlreadyAttached'));
        ...
        var UploadSingleFile = function (file, folder, chunkSize, currentChunk, totalChunks) {
            ...
            for (var i = 0; i < totalChunksSF ; i++) {
                ...
                //    },
                //    error: function () {
                //        alert(VIS.Msg.getMsg('ERRORUploadingFile'));
                //        bsyDiv[0].style.visibility = "hidden";
                //        isRunning=false;
                //    },
                //    success: function (data) {
        ...
        var DownloadFile = function (index, sender) {
            ...
            $.ajax({
                ...
                error: function () {
                    alert(VIS.Msg.getMsg('ERRORGettingFile'));
                    bsyDiv[0].style.visibility = "hidden";
                },
                success: function (data) {
                    //debugger;
