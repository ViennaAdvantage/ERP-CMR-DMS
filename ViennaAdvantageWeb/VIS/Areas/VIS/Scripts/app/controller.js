...
; (function (VIS, $) {
    ...
    function GridWindow(json) {
        ...
        for (var i = 0; i < json._tabs.length; i++) {
            //console.log(json._tabs.length);
            this.tabs.push(new VIS.GridTab(json._tabs[i], this.vo));
        }
 ...
 */
    ...
    GridTab.prototype.getValueAsString = function (variableName) {
        return VIS.context.getWindowContext(this.vo.windowNo, variableName, true);
    };
    ...
    */
    ...
    GridTab.prototype.getTrxInfo = function (tableName, ctx, windowNo, tabNo) {
        if (tableName.startsWith("C_Order") || tableName.startsWith("C_Invoice")) {
            ...
            try {
                dr = VIS.DB.executeDataReader(sql.toString());
                if (dr.read()) {
                    //	{0} - Number of lines
    ...
    GridTab.prototype.getIsProcessed = function () {
        ...
        return "Y".equals(VIS.context.getWindowContext(this.vo.windowNo, "Processed"));
    };	//	
    ...
    GridTab.prototype.validateQuery = function (query) {
        ...
        if (tabKeyColumn.equals("AD_Reference_ID")) {
            ...
            var AD_Reference_ID = VIS.DB.executeScalar(sql, colName);
    ...
    GridTab.prototype.processCallout = function (field) {
        ...
        while (st.hasMoreTokens())      //  for each callout
        {
            ...
            var call = null;
            var method = null;
            cmd = cmd.replace("org.compiere.model", "VAdvantage.Model");
            var methodStart = cmd.lastIndexOf(".");
            try {
                if (methodStart != -1)      //  no class
                {

                    var cClass = cmd.substring(0, methodStart).replace("ViennaAdvantage", "VIS").replace("VAdvantage", "VIS");
    ...
    */
    GridTab.prototype.loadLocks = function () {
        ...
        var sql = "SELECT Record_ID "
            ...
            + "WHERE AD_User_ID=" + AD_User_ID.toString() + " AND AD_Table_ID=" + this.vo.AD_Table_ID.toString() + " AND IsActive='Y' "
            + "ORDER BY Record_ID";
        var dr = null;
        try {
            if (this._Lock == null)
    ...
    GridTab.prototype.loadAttachments = function () {
        ...
        try {
            ...
            dr = VIS.DB.executeDataReader(sqlQry);
            var key, value;
            while (dr.read()) {
                key = VIS.Utility.Util.getValueOfInt(dr.getString(1));
                value = VIS.Utility.Util.getValueOfInt(dr.getString(0));
 ...
 */
    ...
    GridTable.prototype.setValueAt = function (value, row, col, force) {
        //	Can we edit?
        if (!this.isOpen || this.readOnly       //  not accessible
    ...
    GridTable.prototype.open = function (maxRows) {
        ...
        this.rowCount = VIS.DB.executeScalar(this.SQL_Count, null);
        ...
        if (this.rowCount > 0) {
            ...
            //call Query Complete
            this.fireQueryCompleted(true);//Inform GridController   
        }
        return true;
    };
    ...
    GridTable.prototype.dataIgnore = function () {
    ...
    */
    ...
    GridField.prototype.getIsEditable = function (checkContext) {
        //TODO:
        var _vo = this.vo;
        if (this.getIsVirtualColumn())
            return false;
        ...
        if (_vo.IsAlwaysUpdateable && !_vo.tabReadOnly)      //  Zoom
            return true;
        ...
        //  Record is Processed	***
        if (checkContext
            && (ctx.getWindowContext(_vo.windowNo, "Processed").equals("Y")
                || ctx.getWindowContext(_vo.windowNo, "Processing").equals("Y")))
            return false;
        ...
        if (checkContext && !ctx.getWindowContext(_vo.windowNo, "IsActive").equals("Y"))
            return false;
    ...
    GridField.prototype.setValue = function (newValue, inserting) {
        ...
        if (_vo.displayType == DisplayType.Text
            ...
            if (newValue.toString() == "Y" || newValue.toString() == "N") {
                ctx.setWindowContext(_vo.windowNo, _vo.ColumnName, newValue.toString());
            } else {
                ctx.setWindowContext(_vo.windowNo, _vo.ColumnName, newValue.toString().toLowerCase() == "true" ? "Y" : "N");
            }
        }
        ...
        this.forcefirepropchange = this.inserting || _vo.displayType == DisplayType.YesNo
    };
