; (function (VIS, $) {
    ...
    MLookup.prototype.get = function (key) {
        ...
        if (!this.info.isParent && this.info.isValidated && this.allLoaded) {
            return this.getDirect(key, false, true); // cache locally
        }
        if (!this.allLoaded && ($.isEmptyObject(this.lookup))
    ...
    MLookup.prototype.getFromList = function (key) {
        ...
        if (!this.info.isParent && this.info.isValidated && this.allLoaded) {
            return this.getDirectFromList(key, false, true);		//	cache locally    
        }
    ...
    MLookup.prototype.getDirect = function (key, saveInCache, cacheLocal) {
        ...
        try {
            ...
            var dr = DB.executeDataReader(query, param);
            if (dr.read()) {
    ...
    MLookup.prototype.addInList = function (key) {
        ...
        var self = this;
        VIS.DB.executeDataReader(directAllQuery, null, function (dr) {
            try {
                ...
                if (dr.read()) {
                    ...
                    if (isNumber) {
                        ...
                        self.lookupDirectAll[" " + value] = { Value: keyValue, Name: VIS.Utility.encodeText(name) };
                    }
                }
                else {
    ...
    MLookup.prototype.load = function (async, checkCache) {
        ...
        // not validated
        if (!this.info.isValidated) {
            var validation = VIS.Env.parseContext(VIS.context, this.getWindowNo(), this.info.validationCode, false);
        ...
        if (async) {
            ...
            var self = this;
            self.loading = true;
            DB.executeDataReaderPaging(sql, 1, 1000, null, function (dr) {
        ...
        try {
            dr = DB.executeDataReader(sql, null);
    ...
    MLookup.prototype.fillDirectList = function () {
        ...
        var self = this;
        VIS.DB.executeDataReaderPaging(directAllQuery, 1, 1000, null, function (dr) {
            try {
    ...
    */
    ...
    MLocationLookup.prototype.getLocation = function (key, trx) {
        if (key == null)
            return null;
    ...
    MLocatorLookup.prototype.getDirect = function (keyValue, saveInCache, trxName) {
        ...
        var sql = "SELECT Value FROM M_Locator WHERE IsActive='Y' and M_Locator_ID=" + keyValue;
    ...
    MLocatorLookup.prototype.load = function (sync) {
        ...
        try {
            ...
            if (sync) {
                var ds = VIS.DB.executeDataSet(finalSql, sqlParaCount > 0 ? param : null);
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    this.lookup[dt.getRow(i).getCell(0)] = { Key: dt.getRow(i).getCell(0), Name: VIS.Utility.Util.getValueOfString(dt.getRow(i).getCell(1)) };
                }
                ds.dispose();
                ds = null;
                ...
                VIS.DB.executeDataSet(finalSql, sqlParaCount > 0 ? param : null, function (ds) {
                    try {
                        var dt = ds.getTable(0);
                        var len = dt.getRows().length;
    ...
    MAccountLookup.prototype.getData = function (mandatory, onlyValidated, onlyActive, temporary) {
        ...
        var dr = null;
        try {
            dr = DB.executeDataReader(sql.toString(), null);
            while (dr.read())
    ...
    MPAttributeLookup.prototype.get = function (value) {
        ...
        try {
            var dr = VIS.DB.executeDataReader("SELECT Description "
    ...
    MGAttributeLookup.prototype.get = function (value) {
        ...
        try {
            var dr = VIS.DB.executeDataReader("SELECT Description "
                                        + "FROM C_GenattributeSetInstance "
                                        + "WHERE C_GenattributeSetInstance_ID=" + C_GenttributeSetInstance_ID);
            if (dr.read()) {
                description = dr.getString(0);			//	Description
                if (description == null || description.length == 0) {
                    if (VIS.Logging.VLogMgt.getIsLevelFine())
    ...
    VIS.MLookupFactory =
        {
            ...
            getMLookUp: function (ctx, windowNo, Column_ID, AD_Reference_ID) {
                ...
                var validationCode = "";
                //
                var sql = "SELECT c.ColumnName, c.AD_Reference_Value_ID, c.IsParent, vr.Code "
                    + "FROM AD_Column c"
            ...
            getLookup_TableDirEmbed: function (language, columnName, baseTable, baseColumn) {
                ...
                var sql = "SELECT c.ColumnName,c.IsTranslated,c.AD_Reference_ID,c.AD_Reference_Value_ID "
