...
.Append(") WHERE AD_PrintFormatItem_ID=").Append(toID).Append(" AND EXISTS").Append("(")
        ...
        static public MPrintFormat CreateFromTable(Ctx ctx, int AD_Table_ID, object AD_Tab_ID)
        {
            return CreateFromTable(ctx, AD_Table_ID, 0, Convert.ToInt32(AD_Tab_ID));
        }	//	createFromTable
        ...
        /// <param name="AD_PrintFormat_ID">AD_PrintFormat_ID</param>
        /// <returns>print format</returns>
        static public MPrintFormat CreateFromTable(Ctx ctx, int AD_Table_ID, int AD_PrintFormat_ID, int AD_Tab_ID)
        {
            int AD_Client_ID = ctx.GetAD_Client_ID();
