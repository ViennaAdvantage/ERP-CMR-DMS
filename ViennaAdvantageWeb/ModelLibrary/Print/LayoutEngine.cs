 ...
 *  - Don't modify this - Program will someday fail unexpectedly	*/
        ...
        string newPageHtml = "<div style='position:relative;border-style:outset;padding: 10px;padding-top: 20px;overflow:scroll;'>";
        ...
        private PrintElement LayoutTable(MPrintFormat format, PrintData printData, int xOffset)
        {
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                for (int c = 0; c < col; c++)
                {
                    ...
                    {
                        htmlTable.Append("<td>" + HttpContext.Current.Server.HtmlEncode(data[row, c].ToString()) + "</td>");
                    }
                }
                htmlTable.Append("</tr>");
                if (row == rows - 1)
