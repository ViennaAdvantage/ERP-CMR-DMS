...
namespace VAdvantage.Model
{
    public class MScheduler : X_AD_Scheduler, ViennaProcessor
    {
        ...
        private String RunReport(Trx trx)
        {
            ...
            if (m_process.GetAD_ReportFormat_ID() > 0)
            {
                string lang = p_ctx.GetContext("#AD_Language");
                lang = lang.Replace("_", "-");
                //try
                //{
                //    lang = p_ctx.GetContext("LoginLanguage");
                //    lang = lang.Replace("_", "-");
                ...
                if ((m_process.GetAD_ReportFormat_ID() > 0) && (lang == "ar-IQ"))
                {
                    ...
                    re = VAdvantage.ReportFormat.ReportFormatEngine.Get(p_ctx, pi, true);
                }
                else
                {
                    re = VAdvantage.ReportFormat.ReportFormatEngine.Get(p_ctx, pi, false);
                }
            }

            else if (m_process.IsCrystalReport())
            {
                re = new CrystalReport.CrystalReportEngine(p_ctx, pi);
            }
            else
            {
