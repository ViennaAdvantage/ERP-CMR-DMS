    ...
    public class Doc_Allocation : Doc
    {
        ...
        public override List<Fact> CreateFacts(MAcctSchema as1)
        {
            ...
            for (int i = 0; i < _lines.Length; i++)
            {
                ...
                else if (invoice.IsSOTrx())
                {
                    ...
                    if (as1.IsAccrual())
                    {
                        int C_Currecy_ID = DB.GetSQLValue(null, "select c_currency_id from c_invoice  WHERE c_invoice_id     =" + line.GetC_Invoice_ID());
                        string CurrencyDate = " SELECT c_conversiontype_id FROM C_Conversion_Rate" +
                        ...
                        int c_conversiontype_id = DB.GetSQLValue(null, CurrencyDate);
                        ...
                        while (idr.Read())
                        {
                            ...
                            {
                                ConvertedValue = MConversionRate.Convert(GetCtx(), Util.GetValueOfDecimal(idr[0])
                                      , C_Currecy_ID, GetC_Currency_ID(), null, c_conversiontype_id, GetAD_Client_ID(), GetAD_Org_ID());
                            }
                            else
                            {
                                ConvertedValue = Util.GetValueOfDecimal(idr[0]);
                            }
        ...
        private Decimal? CreateCashBasedAcct(MAcctSchema as1, Fact fact, MInvoice invoice, Decimal? allocationSource)
        {
            ...
            Doc_Invoice docInvoice = (Doc_Invoice)Doc.Get(new MAcctSchema[] { as1 },
                MInvoice.Table_ID, invoice.GetC_Invoice_ID(), GetTrx());
