...
namespace VAdvantage.DBPort
{
    class ConvertMap_MSSQL
    {
        ...
        static private void initSQLServer()
        {
            ...
            */
            s_ms.Add("\\bacctBalance\\b", "[compiere].ACCTBALANCE");
            s_ms.Add("\\baddDays\\b", "[compiere].ADDDAYS");
            s_ms.Add("\\bbomPriceLimit\\b", "[compiere].BOMPRICELIMIT");
            s_ms.Add("\\bbomPriceList\\b", "[compiere].BOMPRICELIST");
            s_ms.Add("\\bbomPriceStd\\b", "[compiere].BOMPRICESTD");
            s_ms.Add("\\bbomQtyAvailable\\b", "[compiere].BOMQTYAVAILABLE");
            s_ms.Add("\\bbomQtyOnHand\\b", "[compiere].BOMQTYONHAND");
            s_ms.Add("\\bbomQtyOrdered\\b", "[compiere].BOMQTYORDERED");
            s_ms.Add("\\bbomQtyReserved\\b", "[compiere].BOMQTYRESERVED");
            s_ms.Add("\\bbpartnerRemitLocation\\b", "[compiere].BPARTNERREMITLOCATION");
            s_ms.Add("\\bcharAt\\b", "[compiere].CHARAT");
            s_ms.Add("\\bcurrencyBase\\b", "[compiere].CURRENCYBASE");
            s_ms.Add("\\bcurrencyConvert\\b", "[compiere].CURRENCYCONVERT");
            s_ms.Add("\\bcurrencyRate\\(\\b", "[compiere].CURRENCYRATE(");
            s_ms.Add("\\bcurrencyRound\\b", "[compiere].CURRENCYROUND");
            s_ms.Add("\\bdaysBetween\\b", "[compiere].DAYSBETWEEN");
            s_ms.Add("\\bfirstOf\\b", "[compiere].FIRSTOF");
            s_ms.Add("\\bgetChars\\b", "[compiere].GETCHARS");
            s_ms.Add("\\bgetDaysBetween\\b", "[compiere].GETDAYSBETWEEN");
            s_ms.Add("\\binvoiceDiscount\\b", "[compiere].INVOICEDISCOUNT");
            s_ms.Add("\\binvoiceOpen\\b", "[compiere].INVOICEOPEN");
            s_ms.Add("\\binvoicePaid\\b", "[compiere].INVOICEPAID");
            s_ms.Add("\\bpaymentAllocated\\b", "[compiere].PAYMENTALLOCATED");
            s_ms.Add("\\bpaymentAvailable\\b", "[compiere].PAYMENTAVAILABLE");
            s_ms.Add("\\bpaymentTermDiscount\\b", "[compiere].PAYMENTTERMDISCOUNT");
            s_ms.Add("\\bpaymentTermDueDate\\b", "[compiere].PAYMENTTERMDUEDATE");
            s_ms.Add("\\bpaymentTermDueDays\\b", "[compiere].PAYMENTTERMDUEDAYS");
            s_ms.Add("\\bproductAttribute\\b", "[compiere].PRODUCTATTRIBUTE");

            s_ms.Add("\\bTRUNC_DATE\\b", "[compiere].TRUNC_DATE");
            s_ms.Add("\\bTRUNC2_DATE\\b", "[compiere].TRUNC2_DATE");
            s_ms.Add("\\bDUMP\\b", "[compiere].STRDUMP");
            s_ms.Add("\\bTO_NCHAR\\b", "STR");
            s_ms.Add("\\bTO_CHAR\\b", "STR");
            s_ms.Add("\\bNVL\\b", "COALESCE");

            s_ms.Add("\\bTO_CHAR\\b", "[compiere].TO_CHAR");
            s_ms.Add("\\bTO_DATE\\b", "[compiere].TO_DATE");
            s_ms.Add("\\bTRIM\\b", "[compiere].TRIM");
