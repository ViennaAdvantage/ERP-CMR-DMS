    ...
    public class MRequest : X_R_Request
    {
        ...
        private void SetPriority()
        {
            ...
            if (GetBPartner() != null)
            {
                ...
                if (prioBase != null && !prioBase.Equals(X_C_BP_Group.PRIORITYBASE_Same))
                {
                    ...
                    {
                        tarGetPrio = Convert.ToChar(char.GetNumericValue(tarGetPrio) + 2);
                    }
                    else
                    {
                        tarGetPrio = Convert.ToChar(char.GetNumericValue(tarGetPrio) - 2);
        ...
        protected void SendNotices(List<String> list)
        {
            ...
            String subject = Msg.Translate(GetCtx(), "R_Request_ID")
                + " " + Msg.GetMsg(GetCtx(), "Updated", true) + ": " + GetDocumentNo();
            //	Message
            ...
            try
            {
                ...
                while (idr.Read())
                {
                    ...
                    String NotificationType = idr.GetString(1);
                    if (NotificationType == null)
                        NotificationType = X_AD_User.NOTIFICATIONTYPE_EMail;
                    String email = idr.GetString(2);
                    String Name = idr.GetString(3);
                    //	Role
